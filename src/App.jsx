...
const stripePromise = loadStripe(import.meta.env.VITE_STRIPE_PUBLISHABLE_KEY || "pk_test_51Nf2d6SH8n2b3e4f5g6h7i8j9kLMNOPQRStuvwxyz1234567890");
...
const Navigation = ({ user, onAuthClick, onLogout, userMembership }) => {
  ...
  const getVisibleNavItems = () => {
    if (!user) {
      ...
      return ["HOME", "CLASSES", "TRAINERS", "MEMBERSHIPS", "GALLERY", "CONTACT"];
    }
  };
...
const ClassesSection = ({ user, userMembership, onAuthRequired, showToast }) => {
  const [classes, setClasses] = useState([]);
  const [loading, setLoading] = useState(true);
  const [showPayment, setShowPayment] = useState(false);
  const [selectedClass, setSelectedClass] = useState(null);
  const [bookingQuotas, setBookingQuotas] = useState({ classes: 0, trainers: 0 });
  ...
  else if (planName.includes("standard") || planName.includes("pro") || planName.includes("power")) {
    return { classes: 2, trainers: 2 };
  } 

  else if (planName.includes("premium") || planName.includes("elite") || planName.includes("evolve")) {
  ...
  const handleBookClass = async (classItem) => {
    ...
    if (!error) {
      ...
      setBookingQuotas({ ...bookingQuotas, classes: bookingQuotas.classes + 1 });
    } else {
          ...
          {userMembership && (
            <div className="max-w-2xl mx-auto mb-8 bg-zinc-900/50 border border-zinc-700 rounded-xl p-4 text-center">
              <p className="text-gray-300 text-sm">
                Classes booked this month: <span className="text-red-500 font-bold">{bookingQuotas.classes}</span> / {limits.classes === 999 ? '∞' : limits.classes}
              </p>
            </div>
          )}
              ...
              >
                ...
                <div className="p-6">
                  ...
                  <button
                    ...
                    disabled={!userMembership || bookingQuotas.classes >= limits.classes}
                  ...
                  >
                    {!userMembership ? "MEMBERSHIP REQUIRED" : bookingQuotas.classes >= limits.classes ? "QUOTA REACHED" : "BOOK CLASS"}
                  </button>
                </div>
              </div>
            ))}
...
const TrainersSection = ({ user, userMembership, onAuthRequired, showToast }) => {
  ...
  const handleBookSession = async (trainer) => {
    ...
    if (!error) {
      ...
      setBookingQuotas({ ...bookingQuotas, trainers: bookingQuotas.trainers + 1 });
    } else {
      showToast(`Booking failed: ${error.message}`, "error");
    }
  };
      ...
      <section id="trainers" className="py-20 bg-zinc-950 px-4">
        <div className="max-w-7xl mx-auto">
          ...
          {userMembership && (
            <div className="max-w-2xl mx-auto mb-8 bg-zinc-900/50 border border-zinc-700 rounded-xl p-4 text-center">
              <p className="text-gray-300 text-sm">
                Trainer sessions booked this month: <span className="text-red-500 font-bold">{bookingQuotas.trainers}</span> / {limits.trainers === 0 ? 'Not Available' : limits.trainers}
              </p>
            </div>
          )}
              ...
              >
                ...
                <div className="p-6 text-center">
                  ...
                  <button
                    ...
                    disabled={!userMembership || limits.trainers === 0 || bookingQuotas.trainers >= limits.trainers}
                    className={`w-full py-3 rounded-lg font-bold text-sm uppercase tracking-wider transition-all shadow-lg ${
                      !userMembership || limits.trainers === 0
                        ? "bg-gray-600 text-gray-300 cursor-not-allowed"
...
>
{!userMembership ? "MEMBERSHIP REQUIRED" : limits.trainers === 0 ? "UPGRADE PLAN" : bookingQuotas.trainers >= limits.trainers ? "QUOTA REACHED" : "BOOK SESSION"}
</button>
</div>
</div>
))}
...
const MembershipSection = ({ user, onAuthRequired, showToast, onMembershipChange }) => {
    ...
    {!user && (
      <div className="mb-8 bg-amber-900/20 border-2 border-amber-500 rounded-xl p-6 text-center">
        <div className="flex items-center justify-center space-x-2 mb-2">
          ...
          <h3 className="text-lg font-black text-white uppercase">Login Required</h3>
        </div>
        <p className="text-amber-300 text-sm">
          Please login to send us a message and get assistance with your fitness journey.
        </p>
      </div>
    )}
      ...
      <form onSubmit={handleSubmit}>
        ...
        >
          ...
          <span className={submitting ? "ml-2" : ""}>
            {submitting ? "SENDING" : !user ? "LOGIN TO SEND" : "SEND MESSAGE"}
          </span>
        </button>
      </form>
    </div>
